...
void io_redirection(int ap, int w, char* cwd, char* file_name_redirection, int output_to_file, int output_to_pipe, int write_fd) {
    if (output_to_file == 1) {
        if (ap == 0 && w == 0) {
    ...
    if (output_to_pipe == 1) {
        write(write_fd, global_buffer, strlen(global_buffer));
    }
}
